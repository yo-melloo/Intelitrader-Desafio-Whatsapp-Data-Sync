...
public class RedisService
{
    ...
    public RedisService(IConfiguration config)
    {
        ...
        {
            Console.WriteLine("[RedisService] Conectado ao Redis");
        }
        else
        {
            Console.WriteLine("[RedisService] Erro ao conectar ao Redis");
        }
    ...
    public async Task<List<MessageDto>> GetRecentMessagesAsync()
    {
        ...
        try 
        {
            ...
            Console.WriteLine($"[RedisService] Endpoints encontrados: {string.Join(", ", endpoints.Select(e => e.ToString()))}");
            
            var server = _redis.GetServer(endpoints[0]);
            var keys = server.Keys(pattern: "msg:*").Take(20).ToList();
            Console.WriteLine($"[RedisService] Chaves encontradas (msg:*): {keys.Count}");

            foreach (var key in keys)
            {
                Console.WriteLine($"[RedisService] Processando chave: {key}");
        ...
        {
            Console.WriteLine($"[RedisService] Erro ao buscar mensagens recentes: {ex.Message}");
        }
